...
%      p_vec(m_star_2)=0.8; 
    ...
    Data = Data(trainInd,:);
    [f,F,Data] = get_real_annotations_car(Data,K,M,p_vec,p_block);
%     n_groups = 3;
%     n_collaborator_list = [5,5,5];
%     [F,f] = simulate_missing_blocks_mod2(f,K,p_block,n_groups,n_collaborator_list);
...
%     [rate_err_mv(s)] = majority_voting_label(F_tensor,y,valid_index);
    rate_err_mv(s)=length(u)/N;
    n_err_mv =  length(u);%round(rate_err_mv(s)*length(valid_index));
    time_mv(s)=0;
    % Estimate label accuracy
...
T = table(Proposed_CF,Proposed_CF_EM,Proposed_SVDEig,Proposed_SVDEig_EM,MultiSPA,...
     MultiSPA_EM,MultiSPA_KL,TensorADMM,MV_DS,...
     Minmax_Entropy,KOS,GhoshSVD,EigenRatio,Majority_Voting,'RowNames',rows);

writetable(T,'table_car_block_5.csv','WriteRowNames',true)
