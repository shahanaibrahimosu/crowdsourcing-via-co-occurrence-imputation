...
%   the probability of labelling for each annotator 
     ...
     p_vec = 1*ones(M,1);%(0.2)*rand(M,1);%0.5*ones(M,1);%
%       p_vec(m_star_1)=0.8; % if super annotators are present, higher prob. can be given
%       p_vec(m_star_2)=0.8; 
    %% Generate the data
    [Data,I]    = LoadDataset_credit();
    [rows, col] = size(Data);
    [trainInd,testInd,~] = dividerand(rows,1,0,0);
    Data = Data(trainInd,:);
    [f,F,Data] = get_real_annotations_credit(Data,K,M,p_vec,p_block);
    N = size(Data,1);
    y = Data(:,1);
    pi_vec=ones(K,1)/K;
   ...
   %% Run MultiSPA
    ...
    a = tic;
    lambda_worker = 5*Model.Ndom^2; lambda_task = lambda_worker * (mean(Model.DegWork)/mean(Model.DegTask)); % regularization parameters
    opts={'lambda_worker', lambda_worker, 'lambda_task', lambda_task, 'maxIter',10,'TOL',5*1e-3','verbose',1};
...
rows = {'Error Rate (p=(0.5,0.7))';'std_dev';'time(s)'};
...
writetable(T,'table_credit_5_7_new.csv','WriteRowNames',true)
