function [f,F,TestSet] = get_real_annotations_connect4(Data,K,M,p_vec,p_obs)
...
% TrainSet=getRandomTrainSet(Data);
 [Model8, validationAccuracy8]=trainClassifierCoarsekNNConnect4_num70(TrainSet);
% TrainSet=getRandomTrainSet(Data);
 [Model9, validationAccuracy9]=trainClassifierCoarsekNNConnect4_num50(TrainSet);
%  TrainSet=getRandomTrainSet(Data);
[Model10, validationAccuracy0]=trainClassifierCoarsekNNConnect4_num30(TrainSet);
%  TrainSet=getRandomTrainSet(Data);
%  [Model8, validationAccuracy8]=trainClassifierFineGaussianSVMConnect4(TrainSet);
% TrainSet=getRandomTrainSet(Data);
%  [Model9, validationAccuracy9]=trainClassifierQuadSVMConnect4(TrainSet);
% TrainSet=getRandomTrainSet(Data);
%  [Model10, validationAccuracy10]=trainClassifierLinearSVMConnect4(TrainSet);
%  TrainSet=getRandomTrainSet(Data);
...
% [rows, col] = size(Data);
% for i=1:M
%     [nonobs,obs,~] = dividerand(rows,0.8,0.2,0);
%     f(i,nonobs)=0;
% end
...
[rows, col] = size(Data);
[trainInd,testInd] = dividerand(rows,0.3,0.7);
