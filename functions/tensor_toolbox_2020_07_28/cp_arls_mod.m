function [P,Uinit,output] = cp_arls(X,A,Hinit,R,varargin)
...
params.addParameter('maxepochs', 40);
...
epochsize = ceil((size(X,1)^2)/nsamplsq);%params.Results.epoch;
truefit = params.Results.truefit;

MSE_A= NaN*ones(1,maxepochs);
walltime=NaN*ones(1,maxepochs);
MSE_A(1) = (1/3)*(MSE_measure( A{1},Hinit{1}) + MSE_measure( A{2},Hinit{2}) + MSE_measure( A{3},Hinit{3}));
...
for epoch = 1:maxepochs
    ...
    runtime = toc;
    walltime(:,epoch+1) = runtime + walltime(:,epoch);    
    MSE_A(epoch+1) = (1/3)*(MSE_measure( U{1},A{1}) + MSE_measure( U{2},A{2}) + MSE_measure( U{3},A{3})); 
