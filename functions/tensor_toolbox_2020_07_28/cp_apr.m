function [M, Minit, output] = cp_apr(X,A,A0, R, varargin)
...
elseif strcmp(alg,'pqnr')
    [M, output] = tt_cp_apr_pqnr (X, A, R, Minit, params.Unmatched);
...
function [M, out] = tt_cp_apr_pqnr(X, A, R, Minit, varargin)
...
params.addParamValue('maxiters',10,@(x) isscalar(x) & x > 0);
...
tic;
times(1) = toc;
MSE_A(1) = (1/3)*(MSE_measure( A{1},Minit{1}) + MSE_measure( A{2},Minit{2}) + MSE_measure( A{3},Minit{3}));
...
for iter = 1:maxOuterIters
    ...
    for n = 1:N
        ...
        MSE_A(iter+1) = (1/3)*(MSE_measure( A{1},M{1}) + MSE_measure( A{2},M{2}) + MSE_measure( A{3},M{3}));
