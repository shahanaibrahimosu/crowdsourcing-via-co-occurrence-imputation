...
params.addParameter('maxiters', 40, @isscalar);
...
epciters = ceil((size(X,1)^3)/gsamp);%params.Results.epciters;
...
if use_stoc
    ...
    mse_trace  = zeros(maxiters+1,1);
    fest_trace(1) = fest;
    mse_trace(1)= (1/3)*(MSE_measure(M.U{1},A_gt{1})+MSE_measure(M.U{2},A_gt{2})+MSE_measure(M.U{3},A_gt{3}));%+MSE_measure(A_m.lambda,A_gt.lambda);
    ...
    for nepoch = 1:maxiters      
        ...
        % Added by Shahana
        mse_m= (1/3)*(MSE_measure(M.U{1},A_gt{1})+MSE_measure(M.U{2},A_gt{2})+MSE_measure(M.U{3},A_gt{3}));%+MSE_measure(A_m.lambda,A_gt.lambda);
        
        % Save trace
        fest_trace(nepoch+1) = fest;
        step_trace(nepoch+1) = step;
        mse_trace(nepoch+1) = mse_m;
        ...
        if (printitn > 0) && (mod(nepoch,printitn) == 0 || failed_epoch || festtol_test)
            fprintf('Epoch %2d: f-est = %e, mse = %e, step = %g', nepoch, fest,mse_m, step);
            if failed_epoch
    ...
    info.mse_trace  = mse_trace(1:nepoch+1);
    info.nepoch = nepoch;
