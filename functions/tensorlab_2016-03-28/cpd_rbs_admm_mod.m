function [U, output] = cpd_rbs(T, U, A, varargin)
...
%   - 2014/06/12   NV      Initial version
    ...
    p.addOptional('MaxIter', 350);
    p.addOptional('Algorithm', @AOadmm_shahana);
    p.addOptional('TolFun', 1e-100);
    p.addOptional('TolX', 1e-100);
    p.addOptional('LargeScale', 'auto');
    p.addOptional('TolCRB', 1e-1000);
    p.addOptional('Display', 0);
    %p.addOptional('BlockSize', ones(1, N)*min(R+20,2*R));
    p.addOptional('BlockSize', ones(1, N)*40);
    ...
    MSE_A(1) = (1/3)*(MSE_xiao( U{1},A{1}) + MSE_xiao( U{2},A{2}) + MSE_xiao( U{3},A{3}));
    %MSE_A1(1) = MSE_xiao( U{1},A{1});
    %MSE_A2(1) = MSE_xiao( U{2},A{2});
    %MSE_A3(1) = MSE_xiao( U{3},A{3});
    err_e = normX^2 + norm( ktensor(U) )^2 - 2*innerprod( X, ktensor(U) );
    NRE_A(1) = sqrt(err_e)/normX;
    tic;
    walltime(1)=toc;
    ...
    while ~output.info
        ...
        if useexternalroutines
            [Us, out] = options.Algorithm(Ts, k_admm, ops,XX);
        else 
            [Us, out] = options.Algorithm(XX, k_admm,A, ops,XX);
        end 

        % update original factors
        for n = 1:N, U{n}(idx{n},:) = Us{n}; end

        md = 20;%ceil(size_tens(1)*size_tens(1)*size_tens(1)/(blocksize(1)*blocksize(1)*blocksize(1)*N));
        if mod(k,md)==0
            h(k1) = toc;
            walltime(k1+1) = walltime(k1) +h(k1);
            MSE_A(k1+1) = (1/3)*(MSE_xiao( U{1},A{1}) + MSE_xiao( U{2},A{2}) + MSE_xiao( U{3},A{3}));
            %MSE_A1(k1+1) = MSE_xiao( U{1},A{1});
            %MSE_A2(k1+1) = MSE_xiao( U{2},A{2});
            %MSE_A3(k1+1) = MSE_xiao( U{3},A{3});
            err_e = normX^2 + norm( ktensor(U) )^2 - 2*innerprod( X, ktensor(U) );
            NRE_A(k1+1) = sqrt(err_e)/normX;

            disp(['at iteration ',num2str(k1),' and the MSE is ',num2str(MSE_A(k1+1))]);
            disp(['at iteration ',num2str(k1),' and the NRE is ',num2str(NRE_A(k1+1))]);
            disp(['at iteration ',num2str(k1),' and the TIME is ',num2str(walltime(k1+1))]);
            k1 = k1 + 1;
            tic;
        end
